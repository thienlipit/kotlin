...
public class RSA {
    ...
    public static void main(String[] args) {
        ...
        BigInteger p = largePrime(numBit);
        BigInteger q = largePrime(numBit);
...
//        System.out.println("Restored: " + restoredMessage);
       ...
       // encryption / decryption example
        String message = "Tran thien 123@@,./";
        // Convert string to numbers using a cipher
//        BigInteger cipherMessage = stringCipher(message); //comment this line
        // Encrypt the ciphered message
        List<BigInteger> encrypted = MMencrypt(message, e, n);
        // Decrypt the encrypted message
        String decrypted = MMdecrypt(encrypted, d, n);
        // Uncipher the decrypted message to text
//        String restoredMessage = cipherToString(decrypted);
...
//        System.out.println("Restored: " + restoredMessage);
    ...
    public static BigInteger genE(BigInteger phi) {
        ...
        do {
            e = new BigInteger(numBit*2, rand);
    ...
    public static List<BigInteger> MMencrypt(String msgPlaintext, BigInteger e, BigInteger n) {
        List<BigInteger> ciphertext = new ArrayList<BigInteger>();
        for (char c : msgPlaintext.toCharArray()) {
//            BigInteger temp = BigInteger.valueOf(c).pow(e.intValue()).mod(n);
            BigInteger temp = BigInteger.valueOf(c).modPow(e, n);
            ciphertext.add(temp);
        }
    ...
    public static String MMdecrypt(List<BigInteger> msg, BigInteger d, BigInteger n) {
        StringBuilder msgPlaintext = new StringBuilder();
        for (BigInteger c : msg) {
            BigInteger charCode = c.modPow(d, n);
            msgPlaintext.append((char) charCode.intValue());
        }
